...
high_synonyms = set(
    ["high", "increased", "increase", "elevated", "elevate", "elevation"]
)
abnormal_synonyms = set(
    [
        "abnormal",
        "unusual",
        "atypical",
...
def custom_lemmas(word):
    ...
    if len(word) < 2:
        return returnSet
    if word[-1] == "s":
        returnSet.add(word[:-1])
    if word[-1] == "i":
        returnSet.add(word[:-1] + "us")
    if word[-1] == "a":
    ...
    if word[-3:] == "ata":
        returnSet.add(word[:-2])
    if word[-3:] == "ies":
        returnSet.add(word[:-3] + "y")
    if word[-3:] == "ble":
        returnSet.add(word[:-2] + "ility")
    if len(word) < 7:
        return returnSet
    if word[-6:] == "bility":
        returnSet.add(word[:-5] + "le")
    if len(word) < 8:
        return returnSet
    if word[-7:] == "ication":
...
def sort_ids_by_occurrences_then_earliness(id_to_lines):
    listForm = []
    for hpoid in id_to_lines.keys():
        listForm.append((hpoid, len(id_to_lines[hpoid]), min(id_to_lines[hpoid])))
        ...
        returnString.append(
            "\t".join(
                [
                    ID,
                    names[ID],
                    str(len(safe_ID_to_lines[ID])),
                    str(min(safe_ID_to_lines[ID])),
                    subsent_to_sentence[safe_ID_to_lines[ID].pop()],
                ]
            )
        )
    for ID in unsafe_IDs:
        returnStringUnSafe.append(
            "\t".join(
                [
                    ID,
                    names[ID],
                    str(len(unsafe_ID_to_lines[ID])),
                    str(min(unsafe_ID_to_lines[ID])),
                    subsent_to_sentence[unsafe_ID_to_lines[ID].pop()],
                ]
            )
        )
